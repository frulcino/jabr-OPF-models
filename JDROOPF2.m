function [PGn, QGn, PGe, QGe, cn, sn, un, ce, se, ue, Pn, Qn, Pe, Qe, Fop, Frisk, F] = JDROOPF2(casename,errors_name,Ns, Nxi, beta, rho, Varepsilon)
% Jabr distributionally Robust OPtimal Power Flows
%Difference with JDROPF:
% -implementation of policy norm constraint for the magnitute of power generation to reduce number of CVar
...
%% Distribution Robust Optimization (wasserstein distance = 0.00)
    ...
    Frisk = (sum(sum(sPL)) + sum(sum(sVM + sVm)))/Ns + Varepsilon*(sum(sum(lambdaPL)) + sum(sum(lambdaVM+lambdaVm))); 
    F = Fop +  Frisk;
    minimize F
    subject to
...
% Test
% dualnorm = ones(n_gen,1)
% test = ones(n_gen,3)
% for g = 1:n_gen
%     dualnorm(g) = norm(PGe(g,:)*L,Inf);
%     test(g,1) = dualnorm(g) <= (mpc.gen(g,PMAX) - PGn(g));
%     test(g,2) = dualnorm(g) <= (PGn(g) - mpc.gen(g,PMIN));
% end
