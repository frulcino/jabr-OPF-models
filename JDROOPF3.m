function [PGn, QGn, PGe, QGe, cn, sn, un, ce, se, ue, Pn, Qn, Pe, Qe, Fop, Frisk, F] = JDROOPF3(casename,errors_name,Ns, Nxi, beta, rho, Varepsilon)
% Jabr distributionally Robust OPtimal Power Flows
%Difference with JDROPF2:
% -implementation of policy norm constraint for magnitude of power
...
%uncomment to test function, and comment first and last line:
% clc; clear all; close all;
% [casename, errors_name, Ns, Nxi, beta, rho, Varepsilon] = deal("case118wind3.m","winderrors.mat", 4, 3, 0.02, 400, 0.02);

%[casename, errors_name, Ns, Nxi, beta, rho, Varepsilon] = deal("case118wind3.m","winderrors.mat", 4, 3, 0.01, 400, 0.02)
define_constants;
...
samples = data.S_error; %samples as rows, wind turbines as columns
S_error = samples(1:Ns,:);
...
PGmax = 2*max(abs(samples),[],1); %should be specified in renewable energy specifics (corresponds to maximum power production capacity)
...
%% Distribution Robust Optimization (wasserstein distance = 0.00)
    ...
    for i = 1:n_gen
        Fop = Fop + sum( gencost(i,5)*( PGn(i) + sum(S_error*diag(PGe(i,:)),2) ).^2 + gencost(i,6)*( PGn(i) +  sum(S_error*diag(PGe(i,:)),2)) + gencost(i,7));
    end
    Fop = Fop./Ns;
    % cost of constraints violation (finite reduction)
    % + sum(sum(sPGM+sPGm)))+ sum(sum(lambdaPGM+lambdaPGm)))
    Frisk = (sum(sum(sPL)))/Ns + Varepsilon*(sum(sum(lambdaPL))) ; %+ sum(sum(sVM + sVm))++ sum(sum(lambdaVM+lambdaVm))
    F = Fop +  Frisk;
    minimize F
    subject to
...
for b = 1:n_branch
    ...
    for i = 1:Ns %todo: substitute with matrix multiplication constraint
        rho*((-((Gft(b)+Gtf(b))*cn(b,:)+(Bft(b)-Btf(b))*sn(b,:)-Gff(b)*un(f,:)-Gtt(b)*un(t,:)))+(1-beta)*oPL(b)+(-((Gft(b)+Gtf(b))*ce(b,:)+(Bft(b)-Btf(b))*se(b,:)-Gff(b)*ue(f,:)-Gtt(b)*ue(t,:)))*transpose(S_error(i,:))) <= sPL(b,i);
...
disp(['Adding gen constraints...'])
for g = 1:n_gen
    % PG magnitude policy constraint
    norm(PGe(g,:)*L,Inf) <= mpc.gen(g,PMAX) - PGn(g);          
    norm(PGe(g,:)*L,Inf) <= PGn(g) - mpc.gen(g,PMIN);
end
...
for b = 1:n_bus
    ...
    %policy constraints
        ...
        %voltage magnitude policy constraint
        norm(ue(b)-mpc.bus(b,VMAX)^2*ue(refBus),Inf) <= mpc.bus(b,VMAX)^2*un(refBus) - un(b);
        norm(ue(b)-mpc.bus(b,VMIN)^2*ue(refBus),Inf) <= un(b) - mpc.bus(b,VMIN)^2*un(refBus);
        ...
        if isgen == 1
           %policy power injection constraints
           sum(sum(N .* Pn)) == PGn(gIdx) - mpc.bus(b,PD) - 30; %ho aumentato la domanda di 10 per vedere se i generatori prima o poi cominciano a produrre energia
...
%            for i = 1:Ns %todo: substitute with matrix multiplication constraint
%                rho*((PGn(gIdx) - mpc.gen(gIdx,PMAX))+(1-beta)*oPGM(gIdx)+PGe(gIdx)*transpose(S_error(i,:))) <= sPGM(gIdx,i);
...
%            for i = 1:Ns %todo: substitute with matrix multiplication constraint
%                rho*((-PGn(gIdx) + mpc.gen(gIdx,PMIN))+(1-beta)*oPGm(gIdx)+(-PGe(gIdx))*transpose(S_error(i,:))) <= sPGm(gIdx,i);
%                rho*(-oPGM(gIdx)*beta) <= sPGm(gIdx,i);
%                norm(-rho*(-PGe(gIdx)),Inf) <= lambdaPGm(gIdx);
%            end
...
%         for i = 1:Ns %todo: substitute with matrix multiplication constraint
%             rho*((un(b) - mpc.bus(b,VMAX)^2*un(refBus))+(1-beta)*oVM(b)+(ue(b) - mpc.bus(b,VMAX)^2*ue(refBus))*transpose(S_error(i,:))) <= sVM(b,i);
...
%         for i = 1:Ns %todo: substitute with matrix multiplication constraint
%             rho * ((- un(b) + mpc.bus(b,VMIN)^2*un(refBus)) + (1-beta) * oVm(b)+(- ue(b) + mpc.bus(b,VMIN)^2*ue(refBus))*transpose(S_error(i,:))) <= sVm(b,i);
%             rho*(-oVm(b)*beta) <= sVm(b,i);
...
%% Test
% gen_dualnorm = ones(n_gen,1);
% gen_test = ones(n_gen,2);
% for g = 1:n_gen
%     gen_dualnorm(g) = norm(PGe(g,:)*L,Inf);
%     gen_test(g,1) = gen_dualnorm(g) <= (mpc.gen(g,PMAX) - PGn(g));
%     gen_test(g,2) = gen_dualnorm(g) <= (PGn(g) - mpc.gen(g,PMIN));
% end
% 
% v_dualnorm = ones(n_bus,2);
% v_test = ones(n_bus,2);
%% voltage magnitude policy constraint
% norm(ue(b)-mpc.bus(b,VMAX)^2*ue(refBus),Inf) <= mpc.bus(b,VMAX)^2*un(refBus) - un(b);
% norm(ue(b)-mpc.bus(b,VMIN)^2*ue(refBus),Inf) <= un(b) - mpc.bus(b,VMIN)^2*un(refBus);
% 
% for b = 1:n_bus
%     v_dualnorm(b,1) = norm(ue(b)-mpc.bus(b,VMAX)^2*ue(refBus),Inf);
%     v_dualnorm(b,2) = norm(ue(b)-mpc.bus(b,VMIN)^2*ue(refBus),Inf);
%     v_test(b,1) = v_dualnorm(b,1) <= mpc.bus(b,VMAX)^2*un(refBus) - un(b);
%     v_test(b,2) = v_dualnorm(b,2) <= un(b) - mpc.bus(b,VMIN)^2*un(refBus);
% end
